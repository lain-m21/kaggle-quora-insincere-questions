...
EPOCHS = 5
NUM_SNAPSHOTS = 5
THRESHOLDS = [0.37, 0.37, 0.37, 0.37, 0.37]
...
def tokenize_text(df, logger, tokenizer=None, df_another=None):
    ...
    if tokenizer is None:
        if df_another is None:
            ...
            text_another = df_another['question_text']
            tokenizer = Tokenizer()
            tokenizer.fit_on_texts(list(text) + list(text_another))
...
def train_stacked_rnnfm(seed, seq_train, seq_test, label_train, thresholds, config, embedding_matrix, logger):
    np.random.seed(seed)
...
def main(logger, args):
    ...
    with logger.timer('Tokenize train data'):
        seq_train, tokenizer = tokenize_text(df_train, logger, df_another=df_test)
    ...
    with logger.timer('Load embeddings'):
        embedding_matrix = load_multiple_embeddings(tokenizer.word_index, embed_types=(0, 1, 2), max_workers=2)
    ...
    for i in range(6):
        logger.info(f'Start train and prediction {i + 1} / {6}')
        config = {
            'epochs': EPOCHS,
        ...
        test_preds = train_stacked_rnnfm(
            seed=SEED * i,
            seq_train=seq_train,
            seq_test=seq_test,
