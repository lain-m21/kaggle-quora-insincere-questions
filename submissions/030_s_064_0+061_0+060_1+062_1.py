...
def prepare_seq_and_embeddings(seq_train, seq_test, word_index, embed_types):
    seq_train_pre = pad_sequences(seq_train, PADDING_LENGTH, truncating='pre')
    seq_test_pre = pad_sequences(seq_test, PADDING_LENGTH, truncating='pre')
    seq_train_post = pad_sequences(seq_train, PADDING_LENGTH, truncating='post')
    seq_test_post = pad_sequences(seq_test, PADDING_LENGTH, truncating='post')
    results = load_multiple_embeddings(word_index, embed_types)
    return seq_train_pre, seq_test_pre, seq_train_post, seq_test_post, results
...
class TemporalConvolutionBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2, num_layers=2):
...
class TemporalConvolutionUnit(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2, num_layers=2):
        ...
        for i in range(num_levels):
            ...
            layers += [TemporalConvolutionBlock(in_channels, out_channels, kernel_size, stride=1,
...
class NLPFeaturesConcatDeepRNN(nn.Module):
    def __init__(self, embedding_matrix, seq_len, nlp_size, embed_drop=0.2, mask=False, nlp_factorize=False,
                 ...
                 upper_layer_types=({'dim': 64, 'dropout': 0.5},
                                    {'dim': 64, 'dropout': 0.3})):
        super(NLPFeaturesConcatDeepRNN, self).__init__()
        ...
        if nlp_factorize:
            first_order_rnn_dim = rnn_out_dim * (2 + len(rnn_layer_types) + 1)
            second_order_rnn_dim = rnn_out_dim * sp.special.comb((2 + len(rnn_layer_types) + 1), 2)
        else:
            first_order_rnn_dim = rnn_out_dim * (2 + len(rnn_layer_types)) + nlp_out_dim
            second_order_rnn_dim = rnn_out_dim * sp.special.comb((2 + len(rnn_layer_types)), 2)

        upper_layers = []
        for i, layer_type in enumerate(upper_layer_types):
            if i == 0:
                input_dim = int(first_order_rnn_dim + second_order_rnn_dim)
            else:
                input_dim = upper_layer_types[i - 1]['dim']
            hidden_dim = layer_type['dim']
            dropout = layer_type['dropout']
    ...
    def forward(self, inputs):
        ...
        if self.nlp_factorize:
            x_upper = torch.cat(x_first_order_rnn + x_second_order_rnn, 1)
        else:
            x_upper = torch.cat(x_first_order_rnn + x_second_order_rnn + [x_nlp], 1)
        for i, upper_layer in enumerate(self.upper_layers):
            x_upper = upper_layer(x_upper)
...
def train_nlpdeeprnn(seed, seq_train, seq_test, nlp_train, nlp_test, label_train,
                 thresholds, mode, config, spec, embedding_matrix, logger):
    np.random.seed(seed)
    ...
    with logger.timer('Build NN model'):
        model = NLPFeaturesDeepRNN(embedding_matrix, PADDING_LENGTH, len(nlp_train),
...
def train_nlpcocnatdeeprnn(seed, seq_train, seq_test, nlp_train, nlp_test, label_train,
                           thresholds, mode, config, spec, embedding_matrix, logger):
    np.random.seed(seed)
    with logger.timer('Dataloader preparation'):
        x_train = {
            'text': seq_train.astype(int),
            'nlp': np.concatenate(nlp_train, axis=1)
        }
        x_test = {
            'text': seq_test.astype(int),
            'nlp': np.concatenate(nlp_test, axis=1)
        }
        y_train = label_train.astype(np.float32)

    with logger.timer('Build NN model'):
        model = NLPFeaturesConcatDeepRNN(embedding_matrix, PADDING_LENGTH, len(nlp_train),
...
def train_nlptcnrnn(seed, seq_train, seq_test, nlp_train, nlp_test, label_train,
                    thresholds, mode, config, spec, embedding_matrix, logger):
    np.random.seed(seed)
    with logger.timer('Dataloader preparation'):
        x_train = {
    ...
    with logger.timer('Build NN model'):
        model = NLPFeaturesTCNRNN(embedding_matrix, PADDING_LENGTH, len(nlp_train),
                                  embed_drop=0.2, mask=True,
                                  nlp_layer_types=spec['nlp_layer_types'],
...
def main(logger, args):
    ...
    if TEST:
        logger.info('Extend test data.')
        df_test = pd.concat([df_test for _ in range(7)], axis=0)
    ...
    logger.info('Start multiprocess nlp feature extraction and embedding matrices loading/padding')
    with logger.timer('Parallel nlp feature extraction and embedding matrices loading'), mp.Pool(processes=2) as p:
        results = p.map(parallel_apply, [
            ...
            (prepare_seq_and_embeddings, (seq_train, seq_test, tokenizer.word_index, embed_types))
        ])

    df_extracted_train = results[0][0]
    df_extracted_test = results[0][0]
    seq_train_pre = results[1][0]
    seq_test_pre = results[1][1]
    seq_train_post = results[1][2]
    seq_test_post = results[1][3]
    embedding_matrices = results[1][4]
    ...
    nlp_columns = ['total_length', 'n_capitals', 'n_words', 'n_puncts', 'n_?', 'n_!', 'n_you', 'n_5w1h']
    ...
    # ***** model 1 - 064_0 ***** #
    logger.info(f'Start train and prediction 1 / 4')
    ...
    test_preds = train_nlpcocnatdeeprnn(
        seed=SEED,
        seq_train=seq_train_pre,
        seq_test=seq_test_pre,
    ...
    # ***** model 3 - 062_1 ***** #
    logger.info(f'Start train and prediction 3 / 4')
    ...
    test_preds = train_nlpdeeprnn(
        seed=SEED,
        seq_train=seq_train_post,
        seq_test=seq_test_post,
        nlp_train=x_nlp_train,
        nlp_test=x_nlp_test,
        label_train=label_train,
        thresholds=THRESHOLDS,
    ...
    with logger.timer('Make submission'):
        ...
        if TEST:
            submission.prediction = test_preds_total[:len(submission)]
        else:
