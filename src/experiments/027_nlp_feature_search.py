...
def main(logger, args):
    ...
    model_specs = [['total_length', 'n_capitals', 'n_words'],
                   ['total_length', 'n_capitals', 'n_words', 'n_puncts'],
    ...
    for spec_id, spec in enumerate(model_specs):
        model_name = model_name_base + f'specId={spec_id}_features=({"+".join(spec)})'
        x_continuous = [df_train[col].values.reshape(-1, 1) for col in spec]
        ...
        for fold, (index_train, index_valid) in enumerate(skf.split(label_train, label_train)):
            ...
            model = NLPFeaturesRNNFM({'continuous': len(x_continuous)}, embedding_matrix, PADDING_LENGTH,
                                     hidden_size=64, out_hidden_dim=64, embed_drop=0.2, out_drop=0.3, mask=False,
                                     num_dense_layers=1, dense_activate='relu')
