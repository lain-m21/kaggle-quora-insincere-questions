...
def main(logger, args):
    ...
    model_specs = ['pre', 'post']

    model_name_base = 'StackedRNNFM'

    for spec_id, spec in enumerate(model_specs):
        model_name = model_name_base + f'_specId={spec_id}_truncate={spec}'
        ...
        for fold, (index_train, index_valid) in enumerate(skf.split(label_train, label_train)):
            ...
            seq_train_ = pad_sequences(seq_train, maxlen=PADDING_LENGTH, truncating=spec)
