...
def main(logger, args):
    ...
    model_specs = [{'embed_drop': 0.2, 'out_drop': 0.5, 'out_hidden_dim': 64},
                   {'embed_drop': 0.3, 'out_drop': 0.3, 'out_hidden_dim': 64},
                   {'embed_drop': 0.3, 'out_drop': 0.5, 'out_hidden_dim': 64},
                   {'embed_drop': 0.2, 'out_drop': 0.5, 'out_hidden_dim': 128},
                   {'embed_drop': 0.3, 'out_drop': 0.3, 'out_hidden_dim': 128},
                   {'embed_drop': 0.3, 'out_drop': 0.5, 'out_hidden_dim': 128}]

    model_name_base = 'StackedRNNFM'

    for spec_id, spec in enumerate(model_specs):
        model_name = model_name_base + f'_specId={spec_id}_embeddrop={spec["embed_drop"]}_outdrop={spec["out_drop"]}'
        model_name += f'_outhiddendim={spec["out_hidden_dim"]}'
        ...
        for fold, (index_train, index_valid) in enumerate(skf.split(label_train, label_train)):
            ...
            model = StackedRNNFM(embedding_matrix, PADDING_LENGTH, hidden_size=64, out_hidden_dim=spec['out_hidden_dim'],
                                 embed_drop=spec['embed_drop'], out_drop=spec['out_drop'])
