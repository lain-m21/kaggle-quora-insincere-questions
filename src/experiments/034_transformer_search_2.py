...
def main(logger, args):
    ...
    model_specs = [{'num_head': 2, 'k_dim': 64, 'num_layers': 1, 'dropout': 0.3},
                   {'num_head': 4, 'k_dim': 64, 'num_layers': 1, 'dropout': 0.3},
                   {'num_head': 8, 'k_dim': 64, 'num_layers': 1, 'dropout': 0.3},
                   {'num_head': 2, 'k_dim': 128, 'num_layers': 1, 'dropout': 0.3},
                   {'num_head': 4, 'k_dim': 128, 'num_layers': 1, 'dropout': 0.3}]
    ...
    for spec_id, spec in enumerate(model_specs):
        ...
        for fold, (index_train, index_valid) in enumerate(skf.split(label_train, label_train)):
            ...
            model = TransformerEncoder(embedding_matrix, PADDING_LENGTH, num_layers=spec['num_layers'],
                                       ...
                                       dropout=spec['dropout'], out_drop=0.3, out_hidden_dim=64)
