...
def main(logger, args):
    ...
    model_specs = [{'how': 'mean', 'type': [0, 1, 2], 'embed_drop': 0.2},
                   {'how': 'concat', 'type': [0, 1], 'embed_drop': 0.1},
                   {'how': 'concat', 'type': [0, 1], 'embed_drop': 0.2},
                   {'how': 'concat', 'type': [0, 1, 2], 'embed_drop': 0.2}]
    ...
    for spec_id, spec in enumerate(model_specs):
        ...
        for fold, (index_train, index_valid) in enumerate(skf.split(label_train, label_train)):
            ...
            x_train = {
                ...
                'continuous': [x[index_train] for x in x_nlp]
            }
            x_valid = {
                'text': seq_train[index_valid].astype(int),
                'continuous': [x[index_valid] for x in x_nlp]
            }
            y_train, y_valid = label_train[index_train].astype(np.float32), label_train[index_valid].astype(np.float32)

            model = NLPFeaturesRNN({'continuous': nlp_size}, embedding_matrix, PADDING_LENGTH,
