...
def main(logger, args):
    ...
    model_specs = [{'drop_dir': 0, 'embed_drop': 0.1},
                   {'drop_dir': 0, 'embed_drop': 0.3},
                   {'drop_dir': 1, 'embed_drop': 0.1},
                   {'drop_dir': 1, 'embed_drop': 0.05},
                   {'drop_dir': 1, 'embed_drop': 0.2}]
    ...
    for spec_id, spec in enumerate(model_specs):
        ...
        for fold, (index_train, index_valid) in enumerate(skf.split(label_train, label_train)):
            ...
            model = StackedRNNFM(embedding_matrix, PADDING_LENGTH, hidden_size=64, out_hidden_dim=64,
                                 embed_drop=spec['embed_drop'], out_drop=0.3, embed_drop_direction=spec['drop_dir'])
