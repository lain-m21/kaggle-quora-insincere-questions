...
from model.baseline_rnn import StackedDeepRNN
...
def main(logger, args):
    ...
    if args['debug']:
        ...
        embedding_matrix = load_multiple_embeddings(tokenizer.word_index, embed_types=[0, 2],
    ...
    for factorize in (True, False):
        for out_hidden_dim in (64, 128):
            for seq_drop in (0.0, 0.2):
                for rnn_types in (('lstm', 'gru'),
                                  ('lstm', 'lstm'),
                                  ('lstm', 'gru', 'gru')):
                    model_specs.append({'factorize': factorize,
                                        'out_hidden_dim': out_hidden_dim,
                                        'seq_drop': seq_drop,
                                        'rnn_types': rnn_types})

    model_name_base = 'StackedDeepRNN'

    for spec_id, spec in enumerate(model_specs):
        model_name = model_name_base + f'_specId={spec_id}_factorize={spec["factorize"]}_outhiddendim={spec["out_hidden_dim"]}'
        model_name += f'_seqdrop={spec["seq_drop"]}_rnntypes={spec["rnn_types"]}'
        ...
        for fold, (index_train, index_valid) in enumerate(skf.split(label_train, label_train)):
            ...
            model = StackedDeepRNN(embedding_matrix, PADDING_LENGTH, hidden_size=64, embed_drop=0.2, out_drop=0.3,
                                   layer_norm=True, recurrent_drop=0.0, factorize=spec['factorize'],
                                   out_hidden_dim=spec['out_hidden_dim'], rnn_layer_types=spec['rnn_types'],
                                   seq_drop=spec['seq_drop'])
