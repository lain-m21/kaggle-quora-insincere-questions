...
def main(logger, args):
    ...
    for factorize in (True, False):
        for layer_norm in (True, False):
            for recurrent_drop in (0.0, 0.2):
                for rnn_types in (('lstm', 'lstm'),
                                  ('lstm', 'gru', 'gru'),
                                  ('gru', 'gru', 'gru')):
    ...
    for spec_id, spec in enumerate(model_specs):
        model_name = model_name_base + f'specId={spec_id}_factorize={spec["factorize"]}_layernorm={spec["layer_norm"]}'
        model_name += f'_recurrentdrop={spec["recurrent_drop"]}_rnntypes={spec["rnn_types"]}'
        ...
        for fold, (index_train, index_valid) in enumerate(skf.split(label_train, label_train)):
            ...
            model = StackedDeepRNN(embedding_matrix, PADDING_LENGTH, hidden_size=32, out_hidden_dim=32,
                                   rnn_layer_types=spec['rnn_types'], layer_norm=spec['layer_norm'],
                                   factorize=spec['factorize'], recurrent_drop=spec['recurrent_drop'])
