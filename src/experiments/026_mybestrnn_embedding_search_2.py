...
def main(logger, args):
    ...
    model_specs = [{'name': 'glove+fasttext', 'embeddings': [0, 1]},
                   {'name': 'glove+paragram', 'embeddings': [0, 2]},
                   {'name': 'fasttext+paragram', 'embeddings': [1, 2]},
                   {'name': 'glove+fasttext+paragram', 'embeddings': [0, 1, 2]}]

    model_name_base = 'StackedRNNFM'

    for spec_id, spec in enumerate(model_specs):
        model_name = model_name_base + f'specId={spec_id}_embeddings={spec["name"]}'
        embedding_matrix = np.concatenate([embedding_matrices[i] for i in spec['embeddings']], axis=1)
