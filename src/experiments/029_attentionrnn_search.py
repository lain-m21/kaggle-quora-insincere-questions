...
from model.rnn import AttentionMaskRNN
...
def main(logger, args):
    ...
    model_specs = [{'attention_type': 'general', 'mask': False},
                   {'attention_type': 'dot', 'mask': False},
                   {'attention_type': 'general', 'mask': True},
                   {'attention_type': 'dot', 'mask': True}]
    ...
    for spec_id, spec in enumerate(model_specs):
        ...
        for fold, (index_train, index_valid) in enumerate(skf.split(label_train, label_train)):
            ...
            model = AttentionMaskRNN(embedding_matrix, hidden_size=64, out_hidden_dim=64, embed_drop=0.2, out_drop=0.3,
                                     attention_type=spec['attention_type'], mask=spec['mask'])
