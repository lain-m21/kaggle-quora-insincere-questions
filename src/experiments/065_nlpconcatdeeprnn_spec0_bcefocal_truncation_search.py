...
from model.nlprnn import NLPFeaturesConcatDeepRNN
...
def main(logger, args):
    ...
    model_specs = [
        {'nlp_layer_types': ({'activation': 'relu', 'dim': 16, 'dropout': 0.2},
         ...
         'truncation': 'pre', '5w1h': True, 'nlp_factorize': False},
        {'nlp_layer_types': ({'activation': 'relu', 'dim': 16, 'dropout': 0.2},
                             {'activation': 'relu', 'dim': 16, 'dropout': 0.2}),
         'rnn_layer_types': ({'type': 'lstm', 'dim': 64, 'num_layers': 1, 'dropout': 0.0},
                             {'type': 'gru', 'dim': 64, 'num_layers': 1, 'dropout': 0.0}),
         ...
         'truncation': 'post', '5w1h': True, 'nlp_factorize': False},
    ]
    ...
    for spec_id, spec in enumerate(model_specs):
        ...
        seq_train_ = pad_sequences(seq_train, maxlen=PADDING_LENGTH, truncating=spec['truncation'])

        if spec['5w1h']:
            nlp_features = [col for col in nlp_columns]
        else:
            nlp_features = [col for col in nlp_columns if col != 'n_5w1h']
        x_nlp = [df_train_extracted[col].values.reshape(-1, 1) for col in nlp_features]
        nlp_size = len(x_nlp)
        x_nlp = np.concatenate(x_nlp, axis=1)
        ...
        for fold, (index_train, index_valid) in enumerate(skf.split(label_train, label_train)):
            ...
            x_train = {
                'text': seq_train_[index_train].astype(int),
                'nlp': x_nlp[index_train]
            }
            x_valid = {
                'text': seq_train_[index_valid].astype(int),
                'nlp': x_nlp[index_train]
            }
